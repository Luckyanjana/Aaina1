...
namespace Aaina.Web.Controllers
{
    public class FilterController : BaseController
    {
        ...
        [HttpPost]
        public async Task<IActionResult> Add(FilterDto model, int tenant)
        {
        ...
        private async Task<IActionResult> AddEdit(int? id,int tenant, int? copyId)
        {
            ...
            model.AllRecord = service.GetAll(CurrentUser.CompanyId, CurrentUser.RoleId == (int)UserType.User ? CurrentUser.UserId : (int?)null,tenant);
            if (copyId.HasValue)
            {
                model.Id = null;
            }
        ...
        private async Task<IActionResult> AddEdit(FilterDto model,int tenant)
        {
            ...
            return NewtonSoftJsonResult(new RequestOutcome<string> { RedirectUrl = $"/{tenant}/filter/index", IsSuccess = true });
