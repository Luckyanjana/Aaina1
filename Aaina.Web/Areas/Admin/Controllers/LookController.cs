...
namespace Aaina.Web.Areas.Admin.Controllers
{
    public class LookController : BaseController
    {
        ...
        public IActionResult All(int parentId)
        {
            var all = service.GetAllByGame(CurrentUser.CompanyId, null,parentId);
        ...
        public async Task<IActionResult> AddEdit(int? id, int? parentId, int? copyId)
        {
            ...
            model.GameList = parentId.HasValue ? gameService.GetAllByParentId(parentId.Value, CurrentUser.CompanyId) : new List<GameGridDto>();
            model.AllRecord = model.GameId.HasValue ? service.GetAllByGame(CurrentUser.CompanyId,null, model.GameId.Value) : new List<LookDto>();
        ...
        [HttpPost]
        public async Task<IActionResult> AddEdit(LookDto model)
        {
